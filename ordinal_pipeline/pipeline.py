...
def load_models(path, stage=4, device='cuda', compiled=False, chroma_dpt=False, alb_residual=False):
    ...
    if isinstance(path, str):
        if path in ['paper_weights', 'rendered_only']:
            ...
            ord_state_dict = torch.load(os.path.join(path, 'stage_0_v21.pt'), map_location=device)
            iid_state_dict = torch.load(os.path.join(path, 'stage_1_v21.pt'), map_location=device)
            col_state_dict = torch.load(os.path.join(path, 'stage_2_v21.pt'), map_location=device)
            alb_state_dict = torch.load(os.path.join(path, 'stage_3_v21.pt'), map_location=device)
            dif_state_dict = torch.load(os.path.join(path, 'stage_4_v21.pt'), map_location=device)
            alb_residual = True
